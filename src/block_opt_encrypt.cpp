...
void EncryptBlockOpt::addData(const BytesOpt& dec_data) {
    ...
    if (this->getFreeSpace() < dec_data.getLen()) {
        ...
        PLOG_ERROR << "block data length will exceed the EncryptBlock length (block_len: " 
            << this->block_len << ", current_data_len: " << this->data.getLen() << ", add_data_len: " << dec_data.getLen() << ")";
