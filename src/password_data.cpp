...
bool PasswordData::getData(Bytes bytes) noexcept{
    ...
    while (true){
        ...
        if(!(siteLen == site.value().getLen() && userLen == user.value().getLen() && 
        emailLen == email.value().getLen() && passwordLen == password.value().getLen())){
            this->siteMap.clear();
        ...
        std::vector<std::string> dataV = {
            charVecToString(user.value().getBytes()),
            charVecToString(email.value().getBytes()),
            charVecToString(password.value().getBytes())
        };
...
std::string PasswordData::getError() const noexcept{
    return this->error;
}
